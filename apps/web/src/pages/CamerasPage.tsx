...
const sources: Array<{ type: SourceType; title: string; text: string; icon: typeof CameraIcon; tag?: string }> = [
  ...
  { type: 'browser', title: '手机 / 浏览器摄像头', text: '使用当前浏览器的镜头；只能记录最后看到，不能确认拿起或放下。', icon: Video, tag: '仅观察记录' },
  { type: 'esp32', title: 'ESP32-CAM', text: '读取设备的健康状态、截图和 MJPEG 流。', icon: RadioTower },
...
export function CamerasPage() {
  ...
  return (
    <div>
      <PageHeader title="连接你已有的画面" description="支持电脑摄像头、浏览器直连、标准协议、ESP32-CAM 和明确授权的窗口区域。" actions={<div className="page-actions"><Link className="button secondary" to="/camera-diagnostics"><Activity />摄像头诊断</Link><button className="button primary" onClick={openWizard}><Plus />添加摄像头</button></div>} />
      ...
      {wizard && <Modal title="添加摄像头" description="四步完成连接、命名和位置区域设置。" onClose={() => void closeWizard()} wide>
        ...
        {step === 1 && <div className="wizard-body">
          ...
          {runtime.mode !== 'DEMO' && <p className="photo-help">没有设备也可以先浏览首页示例；示例不会成为真实位置记录。</p>}
        ...
        {step === 2 && <div className="wizard-body connection-step"><div className="selected-source"><span className="metric-icon green">{selectedSource && <selectedSource.icon />}</span><div><strong>{selectedSource?.title}</strong><span>{selectedSource?.text}</span></div></div>
          <div className="form-grid">
            ...
            {form.source_type === 'esp32' && <div className="camera-connection-notice full-field"><strong>ESP32 需要从设备中心接入</strong><p>普通摄像头表单不能把一个地址声明成物理 ESP32。请使用设备中心的 USB 配网 / 安装；没有硬件时仅可在 DEMO 模式使用虚拟设备。</p><Link className="button secondary" to="/devices">前往设备中心</Link></div>}
