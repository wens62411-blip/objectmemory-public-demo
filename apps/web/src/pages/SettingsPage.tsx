...
export function SettingsPage() {
  ...
  return (
    <div>
      <PageHeader eyebrow="偏好与连接" title="让物忆，更适合你的家" description="连接手机、选择识别方式，也决定哪些记忆值得留下。" actions={<button className="button primary" onClick={save} disabled={busy}><Save />{busy ? '正在保存…' : '保存设置'}</button>} />
      ...
      <div className="settings-layout">
        <section className="panel settings-section"><div className="settings-title"><div className="metric-icon green"><ScanLine /></div><div><p className="eyebrow">识别模式</p><h2>选择现场策略</h2></div></div><div className="mode-options"><button className={form.detection_mode === 'aruco' ? 'selected' : ''} onClick={() => setForm((value) => ({ ...value, detection_mode: 'aruco' }))}><ScanLine /><div><strong>稳定标签模式</strong><span>使用 OpenCV ArUco，适合早期验证与现场稳定演示。</span></div><Badge tone="success">推荐</Badge></button><button className={form.detection_mode === 'experimental' ? 'selected' : ''} onClick={() => setForm((value) => ({ ...value, detection_mode: 'experimental' }))}><FlaskConical /><div><strong>无标签 AI 实验模式</strong><span>相似物品、遮挡和低光可能误判；不会阻塞标签模式。</span></div><Badge tone="warning">实验</Badge></button></div></section>

        <section className="panel settings-section"><div className="settings-title"><div className="metric-icon blue"><Gauge /></div><div><p className="eyebrow">性能与事件</p><h2>日常运行参数</h2></div></div><div className="setting-rows"><label><div><strong>物品识别频率</strong><span>每秒分析几次物品，不是画面帧率；预览独立更新，实测帧率请在摄像头页面查看。</span></div><div className="range-control"><input type="range" aria-label="物品识别频率" min={1} max={15} value={form.inference_fps} onChange={(event) => setForm((value) => ({ ...value, inference_fps: Number(event.target.value) }))} /><output>{form.inference_fps} 次/秒</output></div></label><label><div><strong>确认放下等待</strong><span>物品稳定后仍需满足完整移动与证据条件，才会保存确认事件。</span></div><div className="range-control"><input type="range" min={1.5} max={5} step={0.5} value={form.static_seconds} onChange={(event) => setForm((value) => ({ ...value, static_seconds: Number(event.target.value) }))} /><output>{form.static_seconds} 秒</output></div></label><label className="toggle-row"><div><strong>启用本地手部分析</strong><span>分析几何手势和手物共同运动；没有注册照片也可查看手部反馈。不是物品拿放结论。</span></div><input type="checkbox" aria-label="启用本地手部分析" checked={form.hand_detection_enabled !== false} onChange={(event) => setForm((value) => ({ ...value, hand_detection_enabled: event.target.checked }))} /></label><label className="toggle-row"><div><strong>显示手部关键点</strong><span>只控制识别帧上的关键点显示，关闭后仍可继续分析手部动作。</span></div><input type="checkbox" aria-label="显示手部关键点" checked={form.show_hands} onChange={(event) => setForm((value) => ({ ...value, show_hands: event.target.checked }))} /></label></div></section>

        <section className="panel settings-section"><div className="settings-title"><div className="metric-icon coral"><Video /></div><div><p className="eyebrow">事件证据</p><h2>只保存关键前后片段</h2></div></div><div className="setting-rows"><label className="toggle-row"><div><strong>保存新的事件证据</strong><span>关闭后仍做实时检测与轨迹，但不新增截图、短片和事件记录</span></div><input type="checkbox" checked={form.record_events} onChange={(event) => setForm((value) => ({ ...value, record_events: event.target.checked }))} /></label><label className="toggle-row"><div><strong>保存事件短片</strong><span>普通连续录像不会长期保存；仅在启用事件证据时生效</span></div><input type="checkbox" checked={form.save_clips} disabled={!form.record_events} onChange={(event) => setForm((value) => ({ ...value, save_clips: event.target.checked }))} /></label><label><div><strong>事件前缓存</strong><span>事件发生前保留几秒</span></div><select value={form.pre_seconds} disabled={!form.record_events || !form.save_clips} onChange={(event) => setForm((value) => ({ ...value, pre_seconds: Number(event.target.value) }))}><option value={5}>5 秒</option><option value={10}>10 秒</option></select></label><label><div><strong>事件后缓存</strong><span>事件发生后继续记录几秒</span></div><select value={form.post_seconds} disabled={!form.record_events || !form.save_clips} onChange={(event) => setForm((value) => ({ ...value, post_seconds: Number(event.target.value) }))}><option value={5}>5 秒</option><option value={10}>10 秒</option></select></label></div></section>

        <section className="panel settings-section"><div className="settings-title"><div className="metric-icon yellow"><HardDrive /></div><div><p className="eyebrow">本地数据</p><h2>保留与隐私</h2></div></div><div className="setting-rows"><div className="static-setting"><div><strong>统一保留策略</strong><span>默认 MINIMAL：每件物品仅保留最近 2 次未固定的确认变化；清理前可先预览。</span></div><Badge tone="success">MINIMAL</Badge></div><div className="static-setting"><div><strong>匿名轨迹，无人脸身份识别</strong><span>这是固定隐私边界，系统不提供关闭选项</span></div><Badge tone="success">始终启用</Badge></div></div><div className="privacy-summary"><ShieldCheck /><div><strong>完全本地处理</strong><span>家庭视频不上传；摄像头凭据不会写入日志；数据库只保存媒体路径和元数据。</span></div></div><div className="cleanup-actions"><Link className="button secondary" to="/storage"><Database />打开存储管理</Link></div></section>

        {runtime.mode === 'DEMO' && <section className="panel settings-section demo-tools"><div className="settings-title"><div className="metric-icon blue"><PlayCircle /></div><div><p className="eyebrow">演示模式工具</p><h2>准备模拟数据和测试素材</h2></div></div><p>创建的物品、视频和事件均属于 DEMO 数据库，必须标记为模拟来源，不代表真实家庭摄像头识别结果。</p><button className="button secondary" onClick={() => void seed()}><Database />生成演示数据与素材</button></section>}
      </div>
    </div>
  )
}
